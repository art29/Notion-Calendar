...
const Button = ({theme = 'dark', type = 'submit', children, onClick, disabled = false, styling = ''}: ButtonProps) => {
    ...
    return (
        <button className={`${themeStyle()} ${styling}`} type={type} disabled={disabled} onClick={() => onClick && onClick()}>
            {children}
        </button>
    )
}
